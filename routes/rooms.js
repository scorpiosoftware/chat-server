...
module.exports = fp(
  async function (appInstance) {
    ...
    appInstance.get("/api/rooms", {}, async function (req, rep) {
      try {
        const data = await this.db("chat_rooms")
          .join("users", "chat_rooms.admin_id", "=", "users.id")
          .select("chat_rooms.*", "users.name as admin_name");
    ...
    appInstance.get(
      "/api/room/:roomId",
      { preHandler: [appInstance.authenticate] },
    ...
    appInstance.post(
      "/api/room/create",
      { preHandler: [appInstance.authenticateAdmin] },
      async function (req, rep) {
        try {
          const { name, admin_id, room_users } = req.body;
          const [roomId] = await this.db("chat_rooms").insert({
            name,
            admin_id,
          });
          const room = await this.db("chat_rooms")
          ...
          await this.db("users_chat_rooms").del().where({
            room_id: roomId,
          });
    ...
    appInstance.get("/api/chats/:roomId", {}, async function (req, rep) {
      try {
        ...
        const chats = await this.db("messages")
